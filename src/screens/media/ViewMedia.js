...
import { getMedia, deleteMedia } from './ServiceMedia';
...
const ViewMedia = () => {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      await Promise.all(images.map(image => deleteMedia(image.name)));
      setImages([]);
    } catch (error) {
  ...
  const handleDeleteImage = async () => {
    if (currentImageIndex !== null && images[currentImageIndex]) {
      try {
        await deleteMedia(images[currentImageIndex].name);
